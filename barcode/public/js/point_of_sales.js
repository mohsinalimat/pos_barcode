frappe.pages['point-of-sale'].on_page_load = function(wrapper) {
    ...
    frappe.require('assets/js/point-of-sale.min.js', function() {
        wrapper.pos = new barcode.PointOfSale.Controller(wrapper);
        window.cur_pos = wrapper.pos;
    });
  };
  frappe.provide('barcode.PointOfSale')
  
  barcode.PointOfSale.Controller = class {
    constructor(wrapper) {
    ...
    create_opening_voucher() {
      ...
      const dialog = new frappe.ui.Dialog({
        ...
        fields: [
          {
            fieldtype: 'Link', label: ('Company'), default: frappe.defaults.get_default('company'),
            options: 'Company', fieldname: 'company', reqd: 1
          },
          {
            fieldtype: 'Link', label: ('POS Profile'),
        ...
        primary_action: async function({ company, pos_profile, balance_details }) {
          if (!balance_details.length) {
            frappe.show_alert({
              message: ("Please add Mode of payments and opening balance details."),
              indicator: 'red'
            })
  ...
  filters: { company: dialog.fields_dict.company.get_value() }
    ...
    save_draft_invoice() {
      ...
      if (this.frm.doc.items.length == 0) {
        frappe.show_alert({
          message: ("You must add atleast one item to save it as draft."),
          indicator:'red'
        });
  ...
  submit_invoice: () => {
            this.frm.savesubmit()
              .then((r) => {
                ...
                frappe.show_alert({
                  indicator: 'green',
                  message: ('POS invoice {0} created succesfully', [r.doc.name])
                });
              });
          }
        }
  ...
  args: {
    ...
    raise_customer_selection_alert() {
      frappe.dom.unfreeze();
      frappe.show_alert({
        message: ('You must select a customer before adding an item.'),
        indicator: 'orange'
      });
      frappe.utils.play_sound("error");
    }
  ...
  check_serial_batch_selection_needed(item_row) {
    ...
    async check_stock_availability(item_row, qty_needed, warehouse) {
      ...
      if (!(available_qty > 0)) {
        ...
        frappe.throw({
          title: ("Not Available"),
          message: ('Item Code: {0} is not available under warehouse {1}.', [bold_item_code, bold_warehouse])
        })
      } else if (available_qty < qty_needed) {
        frappe.show_alert({
          message: ('Stock quantity not enough for Item Code: {0} under warehouse {1}. Available quantity {2}.', [bold_item_code, bold_warehouse, bold_available_qty]),
          indicator: 'orange'
        });
        frappe.utils.play_sound("error");
      }
    ...
    async check_serial_no_availablilty(item_code, warehouse, serial_no) {
      ...
      if (res.message.includes(serial_no)) {
        frappe.throw({
          title: ("Not Available"),
          message: ('Serial No: {0} has already been transacted into another POS Invoice.', [serial_no.bold()])
        });
      }
    }
    ...
    init_custom_section(){
      this.custom_section = new barcode.PointOfSale.CustomSection({
        wrapper: this.$components_wrapper,
        settings: this.settings,
      })
  ...
  barcode.PointOfSale.CustomSection = class{
  ...
  prepare_dom() {
      this.wrapper.append(
        `<section class="barcode-cart-container customer-cart-container"></section>`
      )
  
      this.$component = this.wrapper.find('.barcode-cart-container');
    }
  
    init_child_components() {
      this.init_barcode_selector();
    }
  
    init_barcode_selector() {
      this.$component.append(
        `<div class="custom-barcode-section customer-section"></div>`
      )
      this.$customer_section = this.$component.find('.custom-barcode-section');
      this.make_barcode_selector();
    }
    make_barcode_selector() {      this.$customer_section.html(`
        <div class="custom-barcode-field"></div>
      `);
      this.custom_barcode_field = frappe.ui.form.make_control({
        df: {
          label: ('Barcode'),
          fieldtype: 'Barcode',
          ...
          onchange: function() {
            if (this.value) {
              ...
              frappe.model.set_value(frm.doc.doctype, frm.doc.name, 'custom_barcode', this.value);
            }
          },
        },
        parent: this.$customer_section.find('.custom-barcode-field'),
        render_input: true,
      });
